...
def brdc_candidate_urls(dt: datetime):
    ...
    # Focusing only on the BKG site, which is known to be stable.
    # We try the compressed .Z first (standard), then the uncompressed .n.
    return [
        f"https://igs.bkg.bund.de/root_ftp/IGS/BRDC/{Y}/{D:03d}/brdc{D:03d}0.{y}n.gz",
        f"https://igs.bkg.bund.de/root_ftp/IGS/BRDC/{Y}/{D:03d}/brdc{D:03d}0.{y}n",
    ]
...
def brdc_fetch_if_needed(train_root: Path, dt: datetime, outdir: Path):
    ...
    for u in brdc_candidate_urls(dt):
        dest = outdir / Path(u).name
        if download_stream(u, dest): 
            # --- START OF DECOMPRESSION FIX ---
            # Apply .gz decompression first if needed
            dest = gunzip_if_needed(dest) 
            # --- END OF DECOMPRESSION FIX ---
            return uncompress_z_if_needed(dest)
...
def process_one(rover_path: Path, train_root: Path, out_root: Path, args):
    ...
    if not nav_local:
        log(f"FAIL: Nav not obtained for {ds_name}"); return
    
    # --- NAVIGATION FILE COPY ADDITION (Crucial for rnx2rtkp) ---
    # Copy the Navigation file to the output directory for rnx2rtkp
    nav_out = outdir / nav_local.name
    if nav_out.resolve() != nav_local.resolve():
        shutil.copy2(nav_local, nav_out)
    # -----------------------------------------------------------
    ...
    return {"dataset": ds_name, "rover": rover_out, "base": base_use, "nav": nav_out}
...
#python ppk_batch_sec_half.py --train-root ".\train_not_run" --out-root ".\ppk_output_2" --station-list "P224,P225,P222,P231,P277"