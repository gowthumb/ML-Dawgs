...
def extract_imu_features(imu_accel_df: pd.DataFrame, imu_gyro_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if accel_cols:
        ...
        # Store 3D components for EKF
        accel_features['accel_x'] = accel_features[x_col]
        accel_features['accel_y'] = accel_features[y_col]
        accel_features['accel_z'] = accel_features[z_col]

        all_imu_features.append(
            accel_features[[
                'millisSinceBoot', 'accel_mag', 'accel_mag_roll_mean',
                'accel_variance', 'accel_jerk', 'accel_mag_no_gravity',
                'accel_x', 'accel_y', 'accel_z'  # Add 3D components
    ...
    if gyro_cols:
        ...
        # Store 3D components for EKF
        gyro_features['gyro_x'] = gyro_features[x_col]
        gyro_features['gyro_y'] = gyro_features[y_col]
        gyro_features['gyro_z'] = gyro_features[z_col]

        all_imu_features.append(
            gyro_features[[
                'millisSinceBoot', 'gyro_mag', 'gyro_mag_roll_mean',
                'gyro_variance', 'gyro_jerk',
                'gyro_x', 'gyro_y', 'gyro_z'  # Add 3D components
            ]].drop_duplicates(subset=['millisSinceBoot'])
        )
...
def extract_pos_features(pos_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # CRITICAL QUALITY FILTERING: Only use high-quality POS solutions
    print(f"POS data before quality filter: {len(pos_df)} epochs")
    if 'quality' in pos_df.columns:
        quality_counts = pos_df['quality'].value_counts().sort_index()
        print(f"Quality distribution: {quality_counts.to_dict()}")

        # Only keep Q=1 (FIX) and Q=2 (FLOAT) - these have <1m accuracy
        # Q=4 (DGPS) and Q=5 (SINGLE) have 10-400m errors!
        features = pos_df[pos_df['quality'].isin([1, 2])].copy()

        print(f"POS data after quality filter (Q=1 or Q=2 only): {len(features)} epochs")

        if features.empty:
            print("WARNING: No high-quality (FIX/FLOAT) POS solutions found!")
            print("   PPK processing may have failed. Check:")
            print("   1. Base station distance (<100km is best)")
            print("   2. RTKLIB configuration (use stricter AR settings)")
            print("   3. Satellite visibility and signal quality")
            return pd.DataFrame({})

        fix_rate = (features['quality'] == 1).sum() / len(features) * 100
        print(f"FIX solution rate: {fix_rate:.1f}%")
        if fix_rate < 20:
            print(f"WARNING: Low FIX rate ({fix_rate:.1f}%) - baseline accuracy may be poor")
    else:
