...
def brdc_fetch_if_needed(train_root: Path, dt: datetime, outdir: Path):
    ...
    if nav:
        return gunzip_if_needed(nav) if nav.suffix.lower()==".gz" else nav
...
def process_one(rover_path: Path, train_root: Path, out_root: Path, args):
    ds_dir  = rover_path.parents[2]
    ds_name = ds_dir.name
    dt      = parse_date_from_dataset_path(ds_dir)  # trusted date from folder name
    ...
    # Base
    seq   = choose_station_sequence(ds_dir, args)
