...
def safe_pdop(H: np.ndarray, W: np.ndarray) -> float:
    # Robust PDOP using pseudoinverse of the weighted normal matrix
    try:
        G = H.T @ W @ H
        Ginv = np.linalg.pinv(G)  # stable if ill-conditioned
        val = float(np.trace(Ginv[:3, :3]))
        return float(math.sqrt(val)) if np.isfinite(val) and val > 0 else float("nan")
    except Exception:
        # Fallback: unweighted geometry
        Q = np.linalg.pinv(H.T @ H)
        val = float(np.trace(Q[:3, :3]))
        return float(math.sqrt(val)) if np.isfinite(val) and val > 0 else float("nan")
...
def solve_epoch_wls_from_rows(rows: pd.DataFrame,
    ...
    # Ensure we have weights from the last iteration
    if w_list is None or len(w_list) != len(sats_df):
        w_list = [1.0] * len(sats_df)
    W = np.diag(w_list)

    try:
        sigma0_2 = float(r.T @ (W @ r) / dof)
        # Weighted covariance (same matrix used for stability)
        P4 = np.linalg.pinv(H.T @ W @ H) * sigma0_2
        P_ecef = P4[:3, :3]
    except Exception:
        # Fall back to NaNs if covariance fails
        P_ecef = np.full((3, 3), np.nan)
    ...
    try:
        ...
        P_enu = np.full((3, 3), np.nan)
        sigE = sigN = sigU = float("nan")

    # Robust PDOP that won't crash on poor geometry
    pdop = safe_pdop(H, W)
